...
- **Unit tests:** `npx ng test --watch=false --browsers=ChromeHeadless --include="<path>/**/*.spec.ts"`. `karma.conf.js` defaults to a visible Chrome in watch mode, so always pass both flags.
  - **Known problem (2026-09, GSIS-28778):** the Karma TypeScript compile failed on about 22 existing broken spec files elsewhere in the app, so the run reported "Executed 0 of 0". The Karma compile covers all of `src`, so this happens for **any** `--include`, however narrow (confirmed on GSIS-9911). On `base-development` at `cafd775d42`, 15 spec files fail to compile (`npx tsc -p tsconfig.spec.json --noEmit` lists them). **Workaround that runs specs** without touching the repo: put a tsconfig outside the repo that `extends` the repo's `tsconfig.json`, sets `types: ["jasmine"]`, and lists in `files` only `src/test.ts`, `src/polyfills.ts` and the target spec(s), plus `include: ["src/**/*.d.ts"]`, and `"typeRoots": ["<repo>/node_modules/@types"]` (without it, a tsconfig outside the repo fails with `TS2688: Cannot find type definition file for 'jasmine'`; GSIS-28093). Then run `npx ng test --watch=false --browsers=ChromeHeadless --ts-config=<that file> --include=<spec>`. If that happens, record it as an environment gap straight away. Don't claim the tests ran, and don't try to fix unrelated specs.
- **Spec style used in this repo:** create the component without TestBed (`Object.create(Component.prototype)`), with dependencies from `jasmine.createSpyObj`. See `review-program-sponsor.component.spec.ts`. That never runs field initializers (e.g. `overriddenValues` arrow functions in list components); for those use `new Component(stubs)` and stub `contextService.getSelectedParentId()` to return `null` so the base list init short-circuits (see `letter-template-add-edit-view.component.spec.ts`). List components' `overriddenValues` may compose display text client-side, e.g. `sponsor-interim-invoice-list.component.ts` joins fee-category names from `invoiceChargeList` (GSIS-28093). That style can't see templates: for a template/DOM defect, use a TestBed spec that imports the feature module's real dependencies (precedent: `resit-retake-rules-list.component.spec.ts`).
