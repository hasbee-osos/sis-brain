...
- **JDK 11 and Gradle.** The checkout has **no Gradle wrapper jar**, so use a local Gradle 8.x (`gradle -p sis-product-sis-admin-backend …`). CI builds with tests skipped (`gradle build -x test`). **The test source set does not currently compile at all** — `gradle test` (even `--tests`-filtered) fails at `:compileTestJava` before any test runs, regardless of which test is targeted (confirmed 2026-09, GSIS-28779: 50 errors across 21 pre-existing files unrelated to finance, as of `base-development` `9b6f9e6d0267c`). A Gradle init script that excludes those 21 files (kept outside the repo, no source modified) lets the rest compile. **Even then, every `@SpringBootTest` fails at context startup** with `BeanDefinitionOverrideException` on bean `restTemplate`, duplicated between `SisAdminServiceApplication` and `ConfigClientAppConfig` (confirmed 2026-09, GSIS-28779, against untouched `StructureMasterServiceIT` and `BatchMasterSpecificationTest` too) — so no `@SpringBootTest`-based test can produce an executed result here until that duplicate bean is fixed. Non-Spring Mockito-based specification tests (e.g. `LetterTemplateSpecificationTest`) do run successfully under the same init script.
- **Known problem (2026-09, GSIS-28778):** in this workspace, `compileJava` stalled in annotation processing (Lombok, MapStruct, Hibernate metamodel) with no compiler error. One attempt ran for hours. **Time-box a build to about 15 minutes.** If nothing finishes, stop and record it as an environment gap; don't leave it running. **The default Gradle daemon heap is too small** for this repo's `compileJava` (GC thrashing, daemon stopped); pass `-Dorg.gradle.jvmargs="-Xmx3g -XX:MaxMetaspaceSize=1g"` on the command line, after which `compileJava` finishes in under 3 minutes (confirmed 2026-09, GSIS-28093).
- **Test-compile init script:** exclude each of the 21 broken test files with `tasks.withType(JavaCompile).configureEach { if (name == 'compileTestJava') exclude '<path>' }` in a Gradle init script kept outside the repo, then pass `--init-script <file>`; plain JUnit tests then compile and run (confirmed 2026-09, GSIS-28093).
